...
//				String strDatevaolam = dateFormat.format(datevaolam);
				...
				if (lastRow >= 0) {
				 ...
				 Object   []obj = {i,zz , tenCN,  datesinh, datevaolam, sdt, diachi, cmndCN,troCap,trangthai };
				 i++;
				modelquanLyCongNhan.addRow(obj);
				}
			}
...
// 
//		            	
//		            	 List<CongDoan> locbangluong=cddao.getAllcongdoantheosanpham(modeldsanphamchamcong.getValueAt(selectedRow,  1).toString());
//		            	 modeldcongdoanchamcong.getDataVector().removeAllElements();
//						for (CongDoan cd : locbangluong) {
//							Object []obj= {cd.getMaCongDoan(),cd.getTenCongDoan(),cd.getGiaCongDoan() };
//							 
						    ...
						    public void propertyChange(PropertyChangeEvent evt) {
						        if ("date".equals(evt.getPropertyName())) {
						            ...
						            System.out.println(ngaycham+"");
						      		...
						      		//
						            modeldDSCongNhanChamCong.getDataVector().removeAllElements();
					         		for (ChamCongCongNhan cccn : cccndao.getDSChamCongCongDoantheoNgay(modeldcongdoanchamcong.getValueAt(selectedRow,  0).toString(),ngaycham,modeldcongdoanchamcong.getValueAt(selectedRow,  3).toString())) {
				...
				if(maCongDoanDonHangtruoc!="" || maCongDoanDonHangtruoc=="") {
					...
					if(maCongDoanDonHangtruoc!=""){
						//nếu tiến độ hiện tại lớn hơn tiến độ của công đoạn trước thì sẽ k thể phân công ,công đoạn trước bn thì công đoạn sau thì dc = hoặc bé hơn
			 			if(cddhdao.tiendocuacongdoandonhangHientai(maCongDoanDonHang) >  cddhdao.tiendocuacongdoandonhangtruoc(maCongDoanDonHangtruoc) ){
			 				modeldDSCongNhanChamCong.setValueAt("",row,4);
			 				System.out.println("êe"+cddhdao.tiendocuacongdoandonhangtruoc(maCongDoanDonHangtruoc));
...
//				    	 else  if(soluong>cddhdao.tiendocuacongdoandonhangtruoc(result)) {
			 			...
			 			else if(soluong >(cddhdao.tiendocuacongdoandonhangtruoc(maCongDoanDonHangtruoc)-cddhdao.tiendocuacongdoandonhangHientai(maCongDoanDonHang))){
			 				modeldDSCongNhanChamCong.setValueAt("",row,4);
			 				JOptionPane.showMessageDialog(null, "Số lượng chấm công vượt qua tiến độ !! Chấm công thất bại!!");
			 				JOptionPane.showMessageDialog(null, "số lượng sản phẩm Chấm công phải dưới hoặc bằng :"+(cddhdao.tiendocuacongdoandonhangtruoc(result)-cddhdao.tiendocuacongdoandonhangHientai(maCongDoanDonHang)));
			 			}
...
//						    	 int zz=setlaisoluongconlai;
				    		 cccndao.updateChamCong(  sogiotangca,soluong  ,localDatengaycham, maChamCong);
				    		 cddhdao.updateTiendo(soluong, maCongDoan, maCongDoanDonHang);
					    	 cddhdao.updatetrangthaiCongDoanDonHang(maCongDoan, maCongDoanDonHang);
	                ...
	                if (!e.getValueIsAdjusting()) {
	                   ...
	                   JTable bangchamcongtheoma = new JTable(modelbangchamcongtheoma) {                     
	                    ...
	                    if (selectedRow >= 0) {
	                            ...
	                            List<ChamCongCongNhan> dsChamCongCNtheoma=cccndao.getchamcongtheomaCn(modelbangluongCongNhan.getValueAt(selectedRow, 1).toString(),thang,nam );
	                			int i1=1;
	                			for (ChamCongCongNhan cccn : dsChamCongCNtheoma) {
                 				 
	                				Object []newData= { cccn.getMaChamCongCongNhan(),cccn.getMaCongNhan().getMaCongNhan(),cccn.getMaCongNhan().getTencongNhan(),cccn.getNgaycham(),cccn.getSoLuong(),cccn.getSogiotangca(),cccn.getMaCongDoan().getMaCongDoan(),cccn.getTiencong() };
	                				i1++;
...
//		            else {
		            	...
		            	for (int row = 0; row < rowCount; row++) {
		            		...
		            		blcndao.updateBangLuong( modelbangluongCongNhan.getValueAt(row, 1).toString());
		            	}
//		            }
